  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/employeeregister/employerjobs?createdBy=${session.user.id}&search=${search}&filter=${filter}&page=${page}&limit=5&sortBy=${sortBy}&sortOrder=${sortOrder}`,
        ...
        <table className="w-full border border-gray-200">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="text-center p-6">No jobs found</td>
              </tr>
            ) : (
              jobs.map((job) => (
                <tr key={job._id} className="border-t">
                  ...
                  <td className="p-3">{job.totalExperience ?? "N/A"}</td>
                  <td className="p-3">
                    <button
                    ...
                    >
                      {actionLoading === job._id ? "..." : job.isActive ? "Active" : "Inactive"}
                    </button>
                  </td>
