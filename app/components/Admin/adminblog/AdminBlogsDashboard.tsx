...
export default function BlogsDashboardPage() {
  ...
  const fetchBlogs = async (resetCursor = false) => {
    try {
      ...
      if (tagFilter.length > 0) params.append("tag", tagFilter.map(t => t.value).join(","));
      params.append("limit", limit.toString());
      if (!resetCursor && nextCursor) params.append("cursor", nextCursor);
      params.append("sortField", sortField);
      params.append("sortOrder", sortOrder);
      ...
      if (resetCursor) setBlogs(data.blogs);
  ...
  const toggleSelectAll = () =>
    setSelectedBlogs(selectedBlogs.length === blogs.length ? [] : blogs.map(b => b._id));
  ...
  // ------------------- Tag Update -------------------
  const handleTagChange = async (blogId: string, tags: string[]) => {
    setBlogs(prev => prev.map(b => (b._id === blogId ? { ...b, tags } : b)));
    try {
      await fetch(`/api/adminblog/${blogId}/tags`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="relative min-h-screen bg-gray-100">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex flex-wrap gap-4 mb-4 sticky top-0 bg-white z-10 p-2 border-b shadow-sm">
              ...
              <CreatableSelect
                ...
                onInputChange={(inputValue) => {
                  if (inputValue.trim() === "") setCategoryFilter(null);
                  else setCategoryFilter({ label: inputValue, value: inputValue });
                  fetchBlogs(true); // trigger live fetch on typing
                }}
              ...
              <CreatableSelect<OptionType, true>
                ...
                options={tagsOptions}
                value={tagFilter}
                onChange={(newValue) => setTagFilter([...newValue])}
                onInputChange={(inputValue) => {
                  // Fetch suggestions or update results while typing
                  fetchBlogs(true);
                }}
                onCreateOption={(inputValue) => {
                  const newTag = { label: inputValue, value: inputValue };
                  setTagsOptions((prev) => [...prev, newTag]);
                  setTagFilter((prev) => [...prev, newTag]);
                  fetchBlogs(true);
                }}
              ...
              <table className="w-full border-collapse mt-2">
                <thead>
                  <tr className="bg-gray-200 cursor-pointer">
                    ...
                    <th onClick={() => handleSort("title")} className="p-2 border">Title</th>
                    <th className="p-2 border">Category</th>
                    <th className="p-2 border">Tags</th>
                    <th className="p-2 border">Status</th>
                    <th onClick={() => handleSort("createdAt")} className="p-2 border">Created At</th>
                    <th className="p-2 border">Author</th>
                    <th className="p-2 border">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {blogs.map((blog) => (
                    <tr key={blog._id} className="hover:bg-gray-50" title={blog.excerpt || ""}>
                      <td className="p-2 border">
                        <input
                          type="checkbox"
                          checked={selectedBlogs.includes(blog._id)}
          ...
          {!formCollapsed && (
            <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex justify-end">
              <div className="w-full md:w-1/3 bg-white p-6 shadow-xl overflow-auto relative animate-slide-in max-h-screen">
                <button
                  onClick={toggleForm}
                  className="absolute top-4 right-4 text-gray-600 hover:text-gray-900"
                >
